...
async function main() {
  ...
  if (idsToDelete.length > 0) {
    ...
    try {
      ...
      console.warn("Analytics deletion note:", e.message);
    }

    console.log(`Deleting ${idsToDelete.length} duplicate questions from production database...`);
  ...
  console.log(`Deduplication complete! Remaining unique questions in DB: ${remaining}`);
}
